...
const EditTransferForm = ({ transfer, submitBtnRef, onSuccess }) => {
  ...
  useEffect(() => {
    if (transfer) {
      setFromLocation(transfer.from_location_id);
      setToLocation(transfer.to_location_id);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      if (!res.isError) {
        ...
        reset();
        onSuccess();
      } else toast.error("Error updating transfer");
    } catch (err) {
    ...
    <div>
      <form onSubmit={handleSubmit(handleTransfer)}>
        <Space direction="vertical" className="w-full">
          ...
          <div className="mt-4">
            <h3 className="mb-2">Transfer Items</h3>
            <div className="flex gap-4 mb-4">
              ...
              <div className="self-end">
                <Button type="primary" onClick={addItem} disabled={!selectedProduct || !quantity}>
                  Add Item
                </Button>
              </div>
            </div>
