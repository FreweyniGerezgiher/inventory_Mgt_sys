...
var A = class _A {
  constructor({ apiKey: t3, authReferrerPolicy: e2, channel: r2, client: n2, id: o2 = "__googleMapsScriptId", language: i2, libraries: s2 = [], mapIds: a2, nonce: l2, region: c2, retries: u2 = 3, url: h2 = "https://maps.googleapis.com/maps/api/js", version: p2 }) {
    if (this.CALLBACK = "__googleMapsCallback", this.callbacks = [], this.done = false, this.loading = false, this.errors = [], this.apiKey = t3, this.authReferrerPolicy = e2, this.channel = r2, this.client = n2, this.id = o2 || "__googleMapsScriptId", this.language = i2, this.libraries = s2, this.mapIds = a2, this.nonce = l2, this.region = c2, this.retries = u2, this.url = h2, this.version = p2, _A.instance) {
      if (!P(this.options, _A.instance.options))
  ...
  createUrl() {
    let t3 = this.url;
    return t3 += `?callback=${this.CALLBACK}`, this.apiKey && (t3 += `&key=${this.apiKey}`), this.channel && (t3 += `&channel=${this.channel}`), this.client && (t3 += `&client=${this.client}`), this.libraries.length > 0 && (t3 += `&libraries=${this.libraries.join(",")}`), this.language && (t3 += `&language=${this.language}`), this.region && (t3 += `&region=${this.region}`), this.version && (t3 += `&v=${this.version}`), this.mapIds && (t3 += `&map_ids=${this.mapIds.join(",")}`), this.authReferrerPolicy && (t3 += `&auth_referrer_policy=${this.authReferrerPolicy}`), t3;
  }
  deleteScript() {
  ...
  setScript() {
    ...
    const t3 = this.createUrl(), e2 = document.createElement("script");
    e2.id = this.id, e2.type = "text/javascript", e2.src = t3, e2.onerror = this.loadErrorCallback.bind(this), e2.defer = true, e2.async = true, this.nonce && (e2.nonce = this.nonce), document.head.appendChild(e2);
  }
  reset() {
  ...
  loadErrorCallback(t3) {
    if (this.errors.push(t3), this.errors.length <= this.retries) {
      ...
      console.log(`Failed to load Google Maps script, retrying in ${t4} ms.`), setTimeout(() => {
...
var j = function(t3) {
  function e2(e3) {
    var r2;
    return (r2 = t3.call(this, e3) || this).loader = null, r2.geocoder = null, "undefined" != typeof window && (r2.loader = new A(e3).load().then(function(t4) {
...
export {
  ...
  C as EsriProvider,
  B as GeoApiFrProvider,
  w as GeoSearchControl,
  D as GeoapifyProvider,
  R as GeocodeEarthProvider,
  j as GoogleProvider,
  N as HereProvider,
  k as JsonProvider,
