...
var PrimaryButton = ({ children, onClick, disabled = false }) => {
  ...
  const ringColor = RING_COLOR.focus[primaryColor];
  const getClassName = (0, import_react.useCallback)(() => {
    return `w-full transition-all duration-300 ${bgColor} ${borderColor}  font-medium border px-4 py-2 text-sm rounded-md focus:ring-2 focus:ring-offset-2 ${bgColorHover} ${ringColor} ${disabled ? " cursor-no-drop" : ""}`;
...
var RoundedButton = ({ children, onClick, disabled, roundedFull = false, padding = "py-[0.55rem]", active = false }) => {
  const { primaryColor } = (0, import_react.useContext)(DatepickerContext);
  const getClassName = (0, import_react.useCallback)(() => {
    const darkClass = "dark:/70 dark:hover:bg-white/10 dark:focus:bg-white/10";
...
var Days = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
  ...
  const activeDateData = (0, import_react.useCallback)((day) => {
    ...
    if ((0, import_dayjs.default)(fullDay).isSame(period.start) && (0, import_dayjs.default)(fullDay).isSame(period.end)) {
      className = ` ${BG_COLOR["500"][primaryColor]}  font-medium rounded-full`;
    } else if ((0, import_dayjs.default)(fullDay).isSame(period.start)) {
      className = ` ${BG_COLOR["500"][primaryColor]}  font-medium ${(0, import_dayjs.default)(fullDay).isSame(dayHover) && !period.end ? "rounded-full" : "rounded-l-full"}`;
    } else if ((0, import_dayjs.default)(fullDay).isSame(period.end)) {
      className = ` ${BG_COLOR["500"][primaryColor]}  font-medium ${(0, import_dayjs.default)(fullDay).isSame(dayHover) && !period.start ? "rounded-full" : "rounded-r-full"}`;
    }
    return {
  ...
  const hoverClassByDay = (0, import_react.useCallback)((day) => {
    ...
    if (dayHover === fullDay) {
      const bgColor = BG_COLOR["500"][primaryColor];
      className = ` transition-all duration-500  font-medium ${bgColor} ${period.start ? "rounded-r-full" : "rounded-l-full"}`;
    }
    return className;
...
var Input = (e) => {
  ...
  const getClassName = (0, import_react.useCallback)(() => {
    ...
    const defaultInputClassName = `relative transition-all duration-300 py-2.5 pl-4 pr-14 w-full border-gray-300 dark:bg-slate-800 dark:/80 dark:border-slate-600 rounded-lg tracking-wide font-light text-sm placeholder-gray-400 bg-white focus:ring disabled:opacity-40 disabled:cursor-not-allowed ${border} ${ring}`;
  ...
  return import_react.default.createElement(
    ...
    import_react.default.createElement(
      ...
      import_react.default.createElement(
        ...
        import_react.default.createElement(
          "div",
          { className: "mt-2.5 shadow-sm border border-gray-300 px-1 py-0.5 bg-white dark:bg-slate-800 dark: dark:border-slate-600 rounded-lg" },
          import_react.default.createElement(
            "div",
